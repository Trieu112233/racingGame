...
Game::Game() : window(sf::VideoMode(980, 800), "Racing Game"), lastScore(0) {
    currentScreen = std::unique_ptr<MenuScreen>(new MenuScreen());
    ...
    while (window.isOpen()) {
        ...
        if (currentScreen->isFinished()) {
            ...
            if (dynamic_cast<MenuScreen*>(currentScreen.get())) {
                if (next == 1) {
                    currentScreen = std::unique_ptr<PlayingScreen>(new PlayingScreen(highScoreManager));
                    mainMenuMusic.stop();
                }
                else if (next == 2) {
                    currentScreen = std::unique_ptr<TopScoreScreen>(new TopScoreScreen(highScoreManager));
                }
                else if (next == 3) {
                    window.close();
                }
                ...
                if (next == 2) { // Game Over
                    ...
                    currentScreen = std::unique_ptr<GameOverScreen>(new GameOverScreen(lastScore));
                    mainMenuMusic.play();
                }
            }
